...
// @version      2.5.3
// @description  Manual column widths, automatic group expansion on load, persistent custom group colors, cell-level color overrides, & group-specific color defaults. 2.5.3 keeps a group's identity stable when Odoo adds a pager & record count to its header.
// @match        https://the-sign-brothers.odoo.com/odoo/all-tasks*
// @updateURL    https://github.com/JPSignBros/OdooScripts-Install/raw/refs/heads/main/DesignBoard%20script.user.js
// @downloadURL  https://github.com/JPSignBros/OdooScripts-Install/raw/refs/heads/main/DesignBoard%20script.user.js
// @grant        none
...
(function () {
    ...
    /*
     * When a group holds more than 80 records, Odoo puts a pager
     * ("1-80 / 82" with two arrows) inside that group's header row.
     * The pager's next-page arrow uses the same chevron icon class as a
     * collapsed group caret, so anything inside the pager has to be
     * ignored when hunting for collapsed groups.
     */
    const PAGER_SELECTOR = ".o_pager, .o_group_pager, [class*='o_pager']";
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            #${COLOR_PANEL_ID} .tyler-remove-group {
                flex: 0 0 auto;
            ...
            #${COLOR_PANEL_ID} .tyler-remove-group:hover {
                border-color: #e8697d;
                background: #e8697d;
                color: #ffffff;
            }
    ...
    /*
     * A group's name has to stay identical no matter how many records it
     * holds, otherwise saved colors detach every time a task is added.
     * Anything count-related is removed: the pager counter ("1-80 / 83"),
     * a bare total ("/ 83"), & the record count in parentheses wherever
     * it appears rather than only at the end.
     */
    ...
    function getGroupName(row) {
        ...
        const sourceElement = preferredNameElement || row;

        /*
         * Read from a detached copy so the pager can be removed without
         * touching the live page.
         */
        const nameSource = sourceElement.cloneNode(true);

        nameSource.querySelectorAll(PAGER_SELECTOR).forEach((node) => {
            node.remove();
        });

        const text = nameSource.textContent || "";
    ...
    function getDefaultColorsForGroup(groupName) {
        ...
        // Copy, so the constants above are never mutated in place.
        return { ...groupDefault };
    }

    /*
     * Read-only. This used to save a default entry for any name it was
     * handed, which is how the color panel filled up with junk. Nothing
     * is written to storage until you actually change a color.
     */
    function getColorsForGroup(groupName) {
        const settings = loadColorSettings();

        return settings[groupName] || getDefaultColorsForGroup(groupName);
    }
    ...
    function updateGroupColor(groupName, field, value) {
        ...
        if (!settings[groupName]) {
            settings[groupName] = getDefaultColorsForGroup(groupName);
        }
     ...
     * CHANGED IN 2.5.1
     *
     * A group holding more than 80 records renders a pager inside its
     * header row, and the pager's next-page arrow carries the same
     * chevron class as a collapsed group caret. Previously the script
     * grabbed that arrow & clicked it, paging the group instead of
     * expanding anything. Candidates inside the pager are now skipped.
     */
        ...
        groupRows.forEach((row) => {
            const candidates = Array.from(
                row.querySelectorAll(
                    ".fa-caret-right, .oi-chevron-right, [aria-expanded='false']"
                )
            );

            const collapsedControl = candidates.find((element) => {
                if (element.closest(PAGER_SELECTOR)) {
                    return false;
                }
            ...
            if (clickable.closest(PAGER_SELECTOR)) {
                return;
            }
    ...
    function buildColorPanel() {
        ...
        groupNames.forEach((groupName) => {
            ...
            removeButton.className = "tyler-remove-group";
            removeButton.textContent = "×";
            removeButton.title =
                "Forget saved colors for this group. Groups still in the " +
                "list will come back with their default colors.";
